...
class ImageDownloader:
    ...
    def _get_image_list(self, ssh_tunnel, database, site, start_date, end_date, **kwargs):
        Logger.info(f'Getting image list from {site} site')
    ...
    def download(self, site, image_list, download_name, url):
        Logger.info(f'Downloading {len(image_list)} images from {site} site')
    ...
    def run(self):
        ...
        if self.kwargs['line_id']:
            ...
            images_list = self._get_image_list(ssh_tunnel, database, **self.kwargs)
            self.download(self.kwargs['site'], images_list, f'{self.kwargs["download_name"]}_{line_id}', url)
            self.server.stop()
...
def parse_opt():
    ...
    parser.add_argument('--site', help='Site name e.g. JQ, ZJ, NK, HZ, etc.')
    parser.add_argument('--line-id', help='Line ID')
    ...
    parser.add_argument('--start-date', help='Start date, e.g. 2021-01-01')
    parser.add_argument('--end-date', default=datetime.now().strftime('%Y-%m-%d'), help='End date, e.g. 2021-01-01')
